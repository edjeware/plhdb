...
--
-- Name: old_plhdb; Type: DATABASE; Schema: -; Owner: postgres
--

CREATE DATABASE old_plhdb WITH TEMPLATE = template0 ENCODING = 'SQL_ASCII' LOCALE = 'C';


ALTER DATABASE old_plhdb OWNER TO postgres;
...
COMMENT ON SCHEMA sandbox IS 'sandbox area for plhdb working group to create tables';
...
CREATE FUNCTION plhdb.get_individual_oid(p_name character varying, p_id character varying, p_sex character, p_birthgroup character varying, p_birthgroup_certainty character, p_is_first_born character, p_study_oid integer, p_study_id character varying, p_oid integer, p_do_dml integer) RETURNS integer
    LANGUAGE plpgsql
    AS $$
DECLARE
        v_ind_oid plhdb.individual.individual_oid%TYPE;
...
ALTER FUNCTION plhdb.get_individual_oid(p_name character varying, p_id character varying, p_sex character, p_birthgroup character varying, p_birthgroup_certainty character, p_is_first_born character, p_study_oid integer, p_study_id character varying, p_oid integer, p_do_dml integer) OWNER TO plhdb_admin;
...
--
-- Name: pg_grant(text, text, text, text); Type: FUNCTION; Schema: plhdb; Owner: plhdb_admin
--

CREATE FUNCTION plhdb.pg_grant(usr text, prv text, ptrn text, nsp text) RETURNS integer
    LANGUAGE plpgsql
    AS $$
 DECLARE
   obj record;
...
ALTER FUNCTION plhdb.pg_grant(usr text, prv text, ptrn text, nsp text) OWNER TO plhdb_admin;

--
-- Name: FUNCTION pg_grant(usr text, prv text, ptrn text, nsp text); Type: COMMENT; Schema: plhdb; Owner: plhdb_admin
--

COMMENT ON FUNCTION plhdb.pg_grant(usr text, prv text, ptrn text, nsp text) IS 'Grants privileges on database or schema objects in bulk. Parameters are the database user or role, 
...
--
-- Name: pg_owner(text, text, text); Type: FUNCTION; Schema: plhdb; Owner: plhdb_admin
--

CREATE FUNCTION plhdb.pg_owner(usr text, ptrn text, nsp text) RETURNS integer
    LANGUAGE plpgsql
    AS $$
 DECLARE
   obj record;
 ...
 BEGIN
   ...
   FOR obj IN SELECT relname FROM pg_class c
     ...
     relname LIKE ptrn
   LOOP
     EXECUTE 'ALTER TABLE ' || nsp || '.' || obj.relname || ' OWNER TO ' || usr;
     num := num + 1;
   END LOOP;
   RETURN num;
 END;
...
ALTER FUNCTION plhdb.pg_owner(usr text, ptrn text, nsp text) OWNER TO plhdb_admin;

--
-- Name: FUNCTION pg_owner(usr text, ptrn text, nsp text); Type: COMMENT; Schema: plhdb; Owner: plhdb_admin
--

COMMENT ON FUNCTION plhdb.pg_owner(usr text, ptrn text, nsp text) IS 'Changed the owner on database or schema objects in bulk. Parameters are the database user or role, 
...
ALTER FUNCTION public.ins_biography_assocs(p_studyid character varying, p_animid character varying, p_animname character varying, p_birthgroup character varying, p_bgqual character, p_sex character, p_momid character varying, p_firstborn character, p_birthdate timestamp without time zone, p_bdmin timestamp without time zone, p_bdmax timestamp without time zone, p_bddist character varying, p_entrydate timestamp without time zone, p_entrytype character varying, p_departdate timestamp without time zone, p_departdateerror double precision, p_departtype character varying, p_anim_oid integer) OWNER TO plhdb_admin;

--
-- Name: pg_grant(text, text, text, text); Type: FUNCTION; Schema: public; Owner: plhdb_admin
--

CREATE FUNCTION public.pg_grant(usr text, prv text, ptrn text, nsp text) RETURNS integer
    LANGUAGE plpgsql
    AS $$
 DECLARE
   obj record;
...
ALTER FUNCTION public.pg_grant(usr text, prv text, ptrn text, nsp text) OWNER TO plhdb_admin;

--
-- Name: FUNCTION pg_grant(usr text, prv text, ptrn text, nsp text); Type: COMMENT; Schema: public; Owner: plhdb_admin
--

COMMENT ON FUNCTION public.pg_grant(usr text, prv text, ptrn text, nsp text) IS 'Grants privileges on database or schema objects in bulk. Parameters are the database user or role, 
 the privilege or (comma-separated) privileges to grant, the text pattern (for LIKE queries) to 
 match schema  objects by, and the name of the schema (public for the default public schema). 
 Returns the number of schema objects to which the privilege(s) were granted.';
...
--
-- Name: pg_revoke(text, text, text, text); Type: FUNCTION; Schema: public; Owner: plhdb_admin
--

CREATE FUNCTION public.pg_revoke(usr text, prv text, ptrn text, nsp text) RETURNS integer
    LANGUAGE plpgsql
    AS $$
 DECLARE
   obj record;
 ...
 BEGIN
   ...
   FOR obj IN SELECT relname FROM pg_class c
     ...
     relname LIKE ptrn
   LOOP
     EXECUTE 'REVOKE ' || prv || ' ON ' || nsp || '.' || obj.relname || ' FROM ' || usr;
     num := num + 1;
   END LOOP;
   RETURN num;
 END;
...
ALTER FUNCTION public.pg_revoke(usr text, prv text, ptrn text, nsp text) OWNER TO plhdb_admin;

--
-- Name: FUNCTION pg_revoke(usr text, prv text, ptrn text, nsp text); Type: COMMENT; Schema: public; Owner: plhdb_admin
--

COMMENT ON FUNCTION public.pg_revoke(usr text, prv text, ptrn text, nsp text) IS 'Revokes privileges on database or schema objects in bulk. Parameters are the database user or role to revoke privileges from, 
 the privilege or (comma-separated) privileges to grant, the text pattern (for LIKE queries) to match schema  objects by, and the
 name of the schema (public for the default public schema). 
 Returns the number of schema objects from which the privilege(s) were revoked.';
...
CREATE TABLE audit.dmltrace (
    ...
    CONSTRAINT dmltrace_optype_check CHECK (((optype = 'I'::bpchar) OR (optype = 'U'::bpchar) OR (optype = 'D'::bpchar)))
);
...
--
-- Name: user_user_oid_seq; Type: SEQUENCE; Schema: auth; Owner: plhdb_admin
--

CREATE SEQUENCE auth.user_user_oid_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
ALTER TABLE auth.user_user_oid_seq OWNER TO plhdb_admin;
...
CREATE TABLE auth.account (
    user_oid integer DEFAULT nextval('auth.user_user_oid_seq'::regclass) NOT NULL,
...
CREATE TABLE auth.permission (
    permission_oid integer DEFAULT nextval('auth.permission_permission_oid_seq'::regclass) NOT NULL,
...
CREATE TABLE plhdb.individual (
    ...
    CONSTRAINT individual_is_first_born_check CHECK (((is_first_born = 'Y'::bpchar) OR (is_first_born = 'N'::bpchar) OR (is_first_born = 'U'::bpchar))),
    CONSTRAINT individual_sex_check CHECK (((sex = 'M'::bpchar) OR (sex = 'F'::bpchar) OR (sex = 'U'::bpchar)))
);
...
--
-- Name: biography_mom_oid; Type: VIEW; Schema: public; Owner: plhdb_admin
--

CREATE VIEW public.biography_mom_oid AS
 SELECT st.id AS studyid,
    an.id AS animid,
    an.name AS animname,
    an.birthgroup,
...
ALTER TABLE public.biography_mom_oid OWNER TO plhdb_admin;
...
--
-- Name: temp; Type: TABLE; Schema: public; Owner: plhdb_admin
--

CREATE TABLE public.temp (
    studyid character varying(12),
    momid character varying(16),
    firstkidbirth timestamp without time zone,
    secondkidbirth timestamp without time zone,
    ibi interval,
    firstkid character varying(16),
    secondkid character varying(16)
);


ALTER TABLE public.temp OWNER TO plhdb_admin;

--
-- Name: temp1; Type: VIEW; Schema: public; Owner: plhdb_admin
--

CREATE VIEW public.temp1 AS
 SELECT biography.studyid,
    biography.momid,
    biography.birthdate AS firstkidbirth,
    biography2.birthdate AS secondkidbirth,
    (biography2.birthdate - biography.birthdate) AS ibi,
    biography.animid AS firstkid,
    biography2.animid AS secondkid
   FROM public.biography,
    public.biography biography2
  WHERE (((biography.studyid)::text = (biography2.studyid)::text) AND ((biography.momid)::text = (biography2.momid)::text) AND (biography2.birthdate > biography.birthdate) AND ((biography.studyid)::text = (1)::text))
  ORDER BY biography.studyid, biography.momid;


ALTER TABLE public.temp1 OWNER TO plhdb_admin;
...
--
-- Name: permission pk_permission_oid; Type: CONSTRAINT; Schema: auth; Owner: plhdb_admin
--

ALTER TABLE ONLY auth.permission
    ADD CONSTRAINT pk_permission_oid PRIMARY KEY (permission_oid);


--
-- Name: account pk_user_oid; Type: CONSTRAINT; Schema: auth; Owner: plhdb_admin
--

ALTER TABLE ONLY auth.account
    ADD CONSTRAINT pk_user_oid PRIMARY KEY (user_oid);
...
CREATE RULE r_femalefertilityinterval_ins AS
    ON INSERT TO public.femalefertilityinterval DO INSTEAD ( INSERT INTO plhdb.recordingperiod (start_oid, end_oid, type_oid, recordingperiod_oid)
  VALUES (plhdb.get_observation_oid(new.startdate, NULL::double precision, plhdb.get_cvterm_oid(NULL::character varying, new.starttype, NULL::character varying, 'event types'::character varying, 'start of recording'::character varying, 0),
        ...
        CASE
            WHEN (new.interval_oid IS NULL) THEN nextval('plhdb.recordingperiod_recordingperiod_oid_seq'::regclass)
            ELSE (new.interval_oid)::bigint
        END);
 INSERT INTO audit.dmltrace (tablename, rowkey, optype, usr, tstamp)
...
--
-- Name: permission fk_permission_user_oid; Type: FK CONSTRAINT; Schema: auth; Owner: plhdb_admin
--

ALTER TABLE ONLY auth.permission
    ADD CONSTRAINT fk_permission_user_oid FOREIGN KEY (user_oid) REFERENCES auth.account(user_oid);
...
ALTER TABLE ONLY plhdb.individual
    ADD CONSTRAINT individual_study_oid_fkey FOREIGN KEY (study_oid) REFERENCES plhdb.study(study_oid) ON DELETE CASCADE;
...
GRANT USAGE ON SCHEMA sandbox TO plhdb_managers;
...
--
-- Name: TABLE biography_mom_oid; Type: ACL; Schema: public; Owner: plhdb_admin
--

GRANT SELECT ON TABLE public.biography_mom_oid TO plhdb_managers;
...
--
-- Name: TABLE temp; Type: ACL; Schema: public; Owner: plhdb_admin
--

GRANT SELECT ON TABLE public.temp TO plhdb_managers;


--
-- Name: TABLE temp1; Type: ACL; Schema: public; Owner: plhdb_admin
--

GRANT SELECT ON TABLE public.temp1 TO plhdb_managers;
